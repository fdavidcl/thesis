...
\begin{figure*}[hbtp]\centering
  ...
  \hfill\includegraphics[width=.3\textwidth,trim={1cm 2cm 0 0},clip]{separable}%
  \hfill\includegraphics[width=.3\textwidth,trim={1cm 2cm 0 0},clip]{mixed}%
  \hfill\includegraphics[width=.3\textwidth,trim={1cm 2cm 0 0},clip]{disjuncts}\hfill%
  \caption{\label{fig.boundaries}Different situations relating to class complexity. The graph on the left shows separable classes, the middle one is an example where classes are not separable and the one on the right shows separable classes with complex boundaries (small disjuncts).}
\end{figure*}
...
Fisher's discriminant ratio is a measure of class overlap, based on the simplest statistics for a distribution, mean and standard deviation. \iffalse It is defined for each feature as in Equation~\ref{eq.fdr}.
  \begin{equation}\label{eq.fdr}\operatorname{dr}(f_i) = \frac{\left(\mu_i^{+}-\mu_i^{-}\right)^2}{\left(\sigma_i^{+}\right)^2+\left(\sigma_i^{-}\right)^2}~,\end{equation}
...
\begin{figure}[ht]
  ...
  \caption{\label{fig.basicae}The essential structure of an AE implemented as a fully connected feed-forward neural network, composed of an encoder $f$ and a decoder $g$. The training loss of this model is measured as the distance $d$ between the input $x$ and its reconstruction $x'=(g\circ f)(x)$.}
\end{figure}
...
\iffalse\begin{table}[ht]
  \caption{\label{tbl.ranking}Average ranking for each method in each evaluated metric. A horizontal line separates complexity metrics from classifier evaluation metrics. The total number of first positions achieved by each method is shown in the last row.}\centering
...
\iffalse\begin{table}[ht]
  \caption{\label{tbl.tests}This table indicates significant differences for each metric between the best method and the rest. The best method is marked with a star \(\bigstar\). Those which are worse with $p<0.05$ are marked with $\times$, and those which are worse with $p<0.01$ are marked with $\otimes$.}\centering
...
\iffalse
  \begin{figure*}[htbp]\centering
    ...
    \caption{\label{fig.critclassifier}These critical distance plots illustrate the statistical tests performed to check significant differences among methods. The first three rows show results for complexity metrics, where methods to the right of each graph are best. The last three rows display classification metrics, where methods are best when located on the left.}
  \end{figure*}
\fi

% Although the previous rankings and statistical tests provide a summary on the comparison among the different methods, we can attempt to gain further insight on the behavior of each one with respect to each metric, by using density estimation plots. These are displayed in Figure~\ref{fig.joyplotscomplexity} for the complexity metrics and in Figure~\ref{fig.joyplotsclassifiers} for the classification metrics. 
