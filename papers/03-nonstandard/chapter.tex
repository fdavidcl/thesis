...
A learning problem is considered to be multi-view\index{multi-view} (MV) \sidecite{mviewl} when inputs are composed of several components of very different nature. 
...
The multi-label (ML)\index{multi-label} learning field \sidecite{mlc,mltutorial} studies problems related to simultaneously assigning multiple labels to a single instance. That is, if $\mathcal L = \{l_1,\dots,l_p\}$ the codomain consists of all possible selections of these $p$ labels, also known as \emph{labelsets}:
\begin{equation}
...
Multi-dimensional (MD)\index{multi-dimensional} learning \sidecite{mdc} is a generalized classification problem where categorization is performed simultaneously along several dimensions. Each instance can belong to one of many classes in each dimension, thus the output space can be formally described as:
\begin{equation}
...
When $Y$ is discrete the problem is usually called \emph{monotone classification} (MC)\index{monotone classification}, monotonic classification or ordinal classification with monotonicity constraints \sidecite{mc-salva}. If, on the contrary, $Y$ is continuous, it is known as \emph{isotonic regression} (IR) \sidecite{ir-book}.

\subsection{Absence or partiality of information}

Some problems do not directly alter the structure of $X$ and $Y$ from the standard supervised problem. Instead, they restrict which data can belong to a training set, or remove labelings from training examples. In this case, training information is presented partially\index{partial information} or with some exclusions.
...
\begin{figure*}[h]
\centering\scriptsize\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[node distance=.6cm, auto]
 %nodes
